...
---
1. Giriş
Günümüzde veri yönetimi, yazılım projelerinin en önemli bileşenlerinden biridir. Geleneksel ilişkisel veritabanlarının yanında NoSQL veritabanları da yaygın olarak kullanılmaktadır. NoSQL veritabanları, esnek yapıları ve yüksek performansları sayesinde özellikle büyük veri uygulamalarında tercih edilmektedir.
Bu çalışmada, bir görev yönetim sistemi için NoSQL veritabanı şeması tasarlanmış ve detaylı bir şekilde açıklanmıştır.

2. NoSQL Veritabanı Hakkında Bilgi
NoSQL veritabanları, verileri tablo yerine farklı yapılarla saklar. Bunlar:
Belge tabanlı (Document-based)


Anahtar-değer (Key-Value)


Sütun tabanlı (Column-based)


Grafik tabanlı (Graph)


Bu projede belge tabanlı (document-based) yapı kullanılmıştır. Veriler JSON benzeri formatta tutulur ve bu sayede daha esnek bir yapı elde edilir.

3. Kullanılan Sistem ve Senaryo
Bu projede bir Görev Yönetim Sistemi (To-Do Uygulaması) ele alınmıştır. Kullanıcılar sisteme kayıt olabilir ve kendilerine ait görevler oluşturabilir.
...
4. Koleksiyonlar ve Veri Yapısı
Kullanıcılar 
{
  _id: ObjectId,
  ad: "Efe",
...
{
  _id: ObjectId,
  baslik: "Veritabanı ödevi",
  aciklama: "NoSQL şeması hazırla",
  durum: "Yapılacak",
  oncelik: "Orta",
  sonTarih: "2026-03-28",
...
5. İlişkiler ve Veri Modelleme
Bu sistemde koleksiyonlar arasında bazı ilişkiler bulunmaktadır:
Bir kullanıcı birden fazla görev oluşturabilir (1-N ilişkisi)


Her görev yalnızca bir kullanıcıya aittir


Görevler kategorilere ayrılabilir


NoSQL veritabanlarında ilişkiler genellikle iki şekilde kurulur:
Referans verme (ID ile bağlama)


Gömme (embedded document)


Bu projede görevler, kullanıcıya kullaniciId ile bağlanmıştır. Kategori bilgisi ise isteğe bağlı olarak görev içerisinde gömülü olarak saklanabilir.

6. Veri Modelleme Yaklaşımları
Referans modelleri
kullaniciId: ObjectId
...
Gömülü model, veri okuma hızını artırırken; referans modeli veri tekrarını azaltır.
...
JSON formatı sayesinde kolay kullanım
...
❌ Dezavantajlar
Karmaşık ilişkilerde zorlanabilir


Veri tekrarına neden olabilir


Standart SQL sorguları yoktur



8. Performans ve Ölçeklenebilirlik
NoSQL veritabanları, büyük veri ile çalışırken yüksek performans sağlar. Yatay olarak ölçeklenebilir, yani sisteme yeni sunucular eklenerek kapasite artırılabilir. Bu özellik, büyük projelerde önemli bir avantajdır.

9. Güvenlik ve Veri Yönetimi
Veritabanında kullanıcı bilgileri saklanırken güvenlik önemlidir. Şifreler düz metin olarak değil, şifrelenmiş şekilde saklanmalıdır. Ayrıca kullanıcı yetkilendirme mekanizmaları da uygulanmalıdır.
